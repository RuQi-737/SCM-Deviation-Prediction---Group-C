...
def export_clean_data(df_dict, output_folder_path, suffix="_cleansed_features"):
    """
    Exports the final, transformed DataFrames to clean CSV files.
    """
    out_path = Path(output_folder_path)

    # Automatically create the 'processed' or 'clean' folder if it doesn't exist
    out_path.mkdir(parents=True, exist_ok=True)

    for key, df in df_dict.items():
        # Construct the file name
        filename = f"Procurement_Data_Company_{key}{suffix}.csv"
        file_path = out_path / filename

        # Export to CSV without the dataframe index
        df.to_csv(file_path, index=False)

        # Print confirmation and file size
        file_size_mb = file_path.stat().st_size / (1024 * 1024)
        print(
            f"Company {key}: Successfully exported to {file_path.name} ({file_size_mb:.2f} MB)")
